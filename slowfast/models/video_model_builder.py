...
from slowfast.models.fusion import Fuser, MLP
...
from .avion.models.model_clip import VideoClassifier, CLIP_VITL14, CLIP_VITB16
...
class Motionformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.txt_proj = nn.Sequential(
            # Dong: mamba 768 hidden states
            nn.Linear(768, self.embed_dim, bias=False),
            # nn.Linear(self.text_model.config.hidden_size, projection_dim, bias=False),
            nn.ReLU(inplace=True), nn.Linear(self.embed_dim, self.embed_dim, bias=True),
            nn.ReLU(inplace=True), nn.Linear(self.embed_dim, self.embed_dim, bias=True)
        )
        # Mamba settings
        if "Mamba" in cfg.FUSION.METHOD:
            expand = 2
            d_state = 16
            d_inner = int(expand * self.embed_dim)
            A = repeat(
            ...
            if "Mamba_disjoint" in cfg.FUSION.METHOD:
                ...
                B_log = torch.log(B)  # Keep A_log in fp32
                self.B_log = nn.Parameter(B_log)
                self.B_log._no_weight_decay = True

            # This module uses roughly 3 * expand * d_model^2 parameters
            self.mamba_video = Mamba(
                                    d_model=self.embed_dim,# Model dimension d_model
                                    d_state=d_state,# SSM state expansion factor
                                    d_conv=4,# Local convolution width
                                    expand=expand,# Block expansion factor
                                    )
            self.mamba_text = Mamba(
                                    d_model=self.embed_dim, # Model dimension d_model
                                    d_state=d_state,  # SSM state expansion factor
                                    d_conv=4,    # Local convolution width
                                    expand=expand,    # Block expansion factor
                                    )
        elif "transformer" in self.cfg.FUSION.METHOD:
            self.fuser = Fuser(dim=self.embed_dim, depth=cfg.FUSION.DEPTH, num_heads=cfg.FUSION.HEADS)
        elif "mlp" in self.cfg.FUSION.METHOD:
            self.fuser = MLP(in_features=self.embed_dim*2, hidden_features=self.embed_dim*2, out_features=self.embed_dim)
    ...
    def forward_features(self, x, metadata): # x: [BS, C=3, T=16, H=224, W=224]
        ...
        # x = self.norm(x)[:, 0]
        x = self.norm(x) # Keep sequence level?
        x = self.pre_logits(x)
    ...
    def forward(self, x, metadata): # x: [BS, C=3, T=16, H=224, W=224]
        # print("Using Motionformer here!!!!!")

        noun_masked_ids = metadata['noun_masked_ids']
        verb_masked_ids = metadata['verb_masked_ids']


        x = self.forward_features(x, metadata) # [BS, N, d]

        if "Mamba" in self.cfg.FUSION.METHOD:
            noun_masked_embeddings = self.text_model.embeddings(input_ids=noun_masked_ids)
            noun_masked_embeddings = self.txt_proj(noun_masked_embeddings)

            verb_masked_embeddings = self.text_model.embeddings(input_ids=verb_masked_ids)
            verb_masked_embeddings = self.txt_proj(verb_masked_embeddings)
            fuse_object = self.mamba_video(x, shared_A=self.A_log)
            
            fuse_noun_masked = self.mamba_text(noun_masked_embeddings, shared_A=self.A_log)
            fuse_verb_maksed = self.mamba_text(verb_masked_embeddings, shared_A=self.A_log)
            x_noun_maksed = fuse_object.mean(dim=1) + fuse_noun_masked.mean(dim=1)
            x_verb_maksed = fuse_object.mean(dim=1) + fuse_verb_maksed.mean(dim=1)
        elif "transformer" in self.cfg.FUSION.METHOD:
            # noun_masked_embeddings = self.text_model.embeddings(input_ids=noun_masked_ids)
            # noun_masked_embeddings = self.txt_proj(noun_masked_embeddings)
            ...
            noun_masked_embeddings = self.text_model(input_ids=noun_masked_ids)
            noun_masked_embeddings = self.txt_proj(noun_masked_embeddings[1])

            verb_masked_embeddings = self.text_model(input_ids=verb_masked_ids)
            verb_masked_embeddings = self.txt_proj(verb_masked_embeddings[1])
            
            x_noun_maksed = self.fuser(x[:, 0].unsqueeze(1), noun_masked_embeddings.unsqueeze(1)).squeeze(1)
            x_verb_maksed = self.fuser(x[:, 0].unsqueeze(1), verb_masked_embeddings.unsqueeze(1)).squeeze(1)
        elif "mlp" in self.cfg.FUSION.METHOD:
            # noun_masked_embeddings = self.text_model.embeddings(input_ids=noun_masked_ids)
            # noun_masked_embeddings = self.txt_proj(noun_masked_embeddings)
            ...
            x_noun_maksed = self.fuser(torch.cat((x[:, 0], noun_masked_embeddings), dim=-1)).squeeze(1)
            x_verb_maksed = self.fuser(torch.cat((x[:, 0], verb_masked_embeddings), dim=-1)).squeeze(1)

        # x_noun_maksed = fuse_noun_masked.mean(dim=1)
        # x_verb_maksed = fuse_verb_maksed.mean(dim=1)


        # x = self.head_drop(x)
        x_noun_maksed = self.head_drop(x_noun_maksed)
        x_verb_maksed = self.head_drop(x_verb_maksed)
        if isinstance(self.num_classes, (list,)) and len(self.num_classes) > 1:
            output = []
            for head in range(len(self.num_classes)):
                if head == 0:
                    x_out = getattr(self, "head%d"%head)(x_verb_maksed)
                else:
                    x_out = getattr(self, "head%d"%head)(x_noun_maksed)
...
class AVION(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.patch_size = cfg.MF.PATCH_SIZE
        self.in_chans = cfg.MF.CHANNELS
        if (cfg.TRAIN.DATASET == "Epickitchensavion") or (cfg.TRAIN.DATASET == "epickitchens"):
            self.num_classes = [97, 300]  
        else:
            self.num_classes = cfg.MODEL.NUM_CLASSES
        self.embed_dim = cfg.MF.EMBED_DIM
        ...
        if "Mamba" in cfg.FUSION.METHOD:
            ...
            self.fuser = Fuser(dim=self.embed_dim)
        elif "mlp" in self.cfg.FUSION.METHOD:
            self.fuser = MLP(in_features=self.embed_dim*2, hidden_features=self.embed_dim*2, out_features=self.embed_dim)
        ...
        ckpt = torch.load("./AVION/checkpoints/avion_pretrain_lavila_vitl_best.pt", map_location='cpu')
        state_dict = OrderedDict()
        for k, v in ckpt['state_dict'].items():
            state_dict[k.replace('module.', '')] = v
        old_args = ckpt['args']
        model = CLIP_VITL14(freeze_temperature=True, use_grad_checkpointing=True, context_length=old_args.context_length, vocab_size=old_args.vocab_size, num_frames=16, drop_path_rate=0.1, use_fast_conv1=True, use_flash_attn=True, use_quick_gelu=True, project_embed_dim=old_args.project_embed_dim,
        ...
        self.orvit = model.visual
        
        if hasattr(model, "image_projection"):
            self.orvit.image_projection = None
...
class AVION_B(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        ckpt = torch.load("./AVION/checkpoints/avion_pretrain_lavila_vitb_best.pt", map_location='cpu')
        state_dict = OrderedDict()
        for k, v in ckpt['state_dict'].items():
            state_dict[k.replace('module.', '')] = v
        old_args = ckpt['args']
        model = CLIP_VITB16(freeze_temperature=True, use_grad_checkpointing=True, context_length=old_args.context_length, vocab_size=old_args.vocab_size, num_frames=16, drop_path_rate=0.1, use_fast_conv1=True, use_flash_attn=True, use_quick_gelu=True, project_embed_dim=old_args.project_embed_dim,
        pretrain_zoo=old_args.pretrain_zoo,
        pretrain_path=old_args.pretrain_path)
        ...
        finetune_checkpoint = torch.load("/nethome/abeedu3/AVION/checkpoints/avion_finetune_cls_lavila_vitb_best.pt", map_location='cpu')
        del model.textual
        for name, param in model.named_parameters():
            if f"module.{name}" in finetune_checkpoint["state_dict"].keys():
                new_param = finetune_checkpoint["state_dict"][f"module.{name}"]
                param = new_param
