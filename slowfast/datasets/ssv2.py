...
class Ssv2(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        elif split == 'standard': # use standard split
            # self.file_labels = f'{self.data_root}/sm/annotations/something-something-v2-labels.json'
            self.file_labels = os.path.join(self.data_root, '20bn-something-something-download-package-labels/labels/labels.json')
            # label_file = f'{self.data_root}/json_files/something-something-v2-{"train" if data_split == "train" else "validation"}.json'
            label_file = os.path.join(self.data_root, f'20bn-something-something-download-package-labels/labels/{"train" if data_split == "train" else "validation"}.json')
        elif split == 'fewshot-base':
            self.file_labels = os.path.join(self.splits_root, f'dataset_splits/fewshot/base_labels.json')
            label_file = os.path.join(self.splits_root, f'20bn-something-something-download-package-labels/label/{"train" if data_split == "train" else "validation"}.json')
        ...
        for video in label_json:
            ...
            objects = video["placeholders"]
            obj_sentence = f"{objects[0]}"
            for i in range(1, len(objects)):
                obj_sentence += f" and {objects[i]}"
            question = "What actions can be done on the objects " + obj_sentence
    ...
    def __getitem__(self, index):
        ...
        verb_question = self._questions[index]
    ...
    def get_frame_path(self, vid_name, frame_idx):
        ...
        :return:
        """
        ppath = os.path.join(os.path.dirname(self.data_root), '20bn-something-something-v2-frames-complete', vid_name, '%06d.jpg' % (frame_idx + 1))
        ppath = os.path.join(self.data_root, '20bn-something-something-v2-frames-complete', vid_name, '%06d.jpg' % (frame_idx + 1))
        return ppath
