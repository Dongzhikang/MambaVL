...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _vid_idx, meta) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            # Compute the loss.
            if (cfg.TRAIN.DATASET == 'epickitchens') or (cfg.TRAIN.DATASET == "Epickitchensavion"):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if isinstance(labels, (dict,)) and ((cfg.TRAIN.DATASET == "epickitchens") or (cfg.TRAIN.DATASET == "Epickitchensavion")) :
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    # Log epoch stats.
    _, stats = val_meter.log_epoch_stats(cur_epoch)
    # add average precision to the tensorboard
    if isinstance(labels, (dict,)) and ((cfg.TRAIN.DATASET == "epickitchens") or (cfg.TRAIN.DATASET == "Epickitchensavion")):
        if writer is not None:
            writer.add_scalars(
                        {
                            "Val/epoch_verb_top1_acc": stats['verb_top1_acc'],
                            "Val/epoch_verb_top5_acc": stats['verb_top5_acc'],
                            "Val/epoch_noun_top1_acc": stats['noun_top1_acc'],
                            "Val/epoch_noun_top5_acc": stats['noun_top5_acc'],
                            "Val/epoch_action_top1_acc": stats['top1_acc'],
                            "Val/epoch_action_top5_acc": stats['top5_acc'],
                        },
                        global_step=len(val_loader) * cur_epoch + cur_iter,
                    )
    # write to tensorboard format if available.
...
def train(cfg):
    ...
    # Build the video model and print model statistics.
    model = build_model(cfg)
    # # Only train mambe parameters.
    # for name, p in model.named_parameters():
    #     if ("A_log" in name) or ("mamba" in name) or ("txt_proj" in name):
