...
class EpicKitchensVideoRecord(VideoRecord):
    ...
    def label(self):
        return {'verb': self._series['verb_class'] if 'verb_class' in self._series else -1,
                ...
                # 'gpt_desc': self._series['gpt_desc'] if 'gpt_desc' in self._series else -1,
                'verb_question': self._series['verb question'] if 'verb question' in self._series else -1,
                'noun_question': self._series['noun question'] if 'noun question' in self._series else -1,
                'verb_text': self._series['verb'] if 'verb' in self._series else -1,
                'noun_text': self._series['noun'] if 'noun' in self._series else -1,}
